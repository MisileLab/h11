...
**Generated:** 2026-02-05T15:33:03Z
**Commit:** ec9df29
**Branch:** main

## OVERVIEW
Monorepo for Schale (Astro SSR site) plus CI/tooling. This checkout only includes the Schale product.

## STRUCTURE
```
h11/
├── .github/     # CI workflows + funding
├── .opencode/   # OpenCode tooling config
└── schale/      # Astro SSR site + scripts
```
...
| CI workflows | `.github/workflows` | Docker publish + agent automation |

## CONVENTIONS
- Schale uses Yarn 4 with `nodeLinker: node-modules` (see `schale/frontend/.yarnrc.yml`).
- Tailwind v4 via Vite (Schale).
...
- Must do commit after implementing one fix/feature.

## ANTI-PATTERNS (THIS PROJECT)
- Do not commit local Python environments like `.venv`.
- Do not commit build outputs like `schale/frontend/dist`.
...
- Docker publish gated by `env.ACT != 'true'`.
- `corin/` and `gqldb/` directories are not present in this checkout.
